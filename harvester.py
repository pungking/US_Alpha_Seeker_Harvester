...
def find_file_id(name, parent_id=None):
    """모든 옵션을 동원하여 파일을 찾아냅니다."""
    for _ in range(3):
        try:
            query = f"name = '{name}' and trashed = false"
            if parent_id: query += f" and '{parent_id}' in parents"
            # supportsAllDrives와 includeItemsFromAllDrives가 있어야 타인 소유 폴더 내 검색 가능
...
def upload_json(filename, data, parent_id):
    """용량 제한(403)을 우회하는 가장 강력한 업로드 방식"""
    ...
    for attempt in range(5):
        try:
            ...
            print(f"   ⚠️ 업로드 에러 ({attempt+1}/5): {str(e)}")
            time.sleep(10)
...
def sync_ohlcv_incremental(ticker, ohlcv_dir_id):
    ...
    try:
        ...
        new_records = [{"date": d.strftime('%Y-%m-%d'), "open": round(r['Open'], 2), "high": round(r['High'], 2), "low": round(r['Low'], 2), "close": round(r['Close'], 2), "volume": int(r['Volume'])} for d, r in df.iterrows()]
...
def run_harvester():
    ...
    try:
            ...
            if s3_files:
                ...
                if ready_info.get("trigger_file") != latest_s3['name']:
                    ...
                    if s3_tickers:
                        ...
                        signal = {"status": "COMPLETED", "stage": 4, "trigger_file": latest_s3['name'], "timestamp": today_str, "count": s3_count}
        ...
        send_telegram(f"📡 *정기 가동:* `{group_label}` | `{len(filtered_tickers)}`종목")
                ...
                try:
                    ...
                    if price:
                        raw_record = {
                            ...
                            "marketCap": info.get('marketCap'), "updated": today_str, "Hist": hist_status,
