...
if not RAW_SERVICE_ACCOUNT:
    print("❌ 서비스 계정 설정(Secrets)이 없습니다."); exit(1)
...
def run_harvester():
    ...
    is_weekend_update = (now_kst.weekday() == 5) # 토요일 여부 체크
    
    send_telegram(f"🤖 *Alpha Seeker v2 가동* ({today_str})\n- 히스토리 갱신 모드: {'활성(토요일)' if is_weekend_update else '조건부'}")
    ...
    try:
        ...
        for ticker in filtered_tickers:
            ...
            try:
                ...
                # [히스토리 업데이트 판단]
                # 기존에 ❌였거나, 오늘이 토요일(정기 갱신일)이면 수집 시도
                current_hist_status = daily_storage[daily_name].get(ticker, {}).get('Hist', '❌')
                
                if current_hist_status == '❌' or is_weekend_update:
                    try:
        ...
        # 3. 구글 드라이브 최종 업로드
        send_telegram(f"📤 *저장 단계*: 데이터 업로드 중...")
